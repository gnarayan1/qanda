...
public class ApplicationTests {
	...
	public void shouldReturnRepositoryIndex() throws Exception {
		...
		mockMvc.perform(get("/")).andDo(print()).andExpect(status().isOk()).andExpect(
				jsonPath("$._links.questions").exists());
		
		mockMvc.perform(get("/")).andDo(print()).andExpect(status().isOk()).andExpect(
				jsonPath("$._links.answers").exists());
	}
	...
	public void shouldRetrieveEntity() throws Exception {
		...
		MvcResult mvcResult = mockMvc.perform(post("/questions").content(
				"{\"text\": \"What's your name?\"}")).andExpect(
						status().isCreated()).andReturn();

		String txt = mvcResult.getResponse().getHeader("Location");
		mockMvc.perform(get(txt)).andExpect(status().isOk()).andExpect(
				jsonPath("$.text").value("What's your name?"));
	}
	...
	public void shouldCreateEntity() throws Exception {
		...
		mockMvc.perform(post("/survey").content(
				"{\"text\": \"Who are you?\"}")).andExpect(
						status().isCreated()).andExpect(
								header().string("Location", containsString("What's your name?")));
	}
